...
- [ ] **Move derivation logic into selectors/client**: invitation status partitioning (`app/ui/invitations-section.tsx:41-43, 201-206`, `inbox-section.tsx:20-27`), granted/candidates (`workspace-access-section.tsx:194-195`), `remoteOf` config unwrapping (`workspace-page.tsx:313-317`), `initials` (`app-shell.tsx:79-86`), `locationOf` (`app/ui/project-location.ts`). **Warning:** the array _filters_ (invitation partitioning, granted/candidates) must not become plain selectors — a selector returning a fresh array breaks `useSyncExternalStore` (see `app/ui/workspace-page.tsx:87-88` and client rule 4); only the pure per-item functions (`remoteOf`, `initials`, `locationOf`) move safely today, and the filters need a memoization design first.
...
- [ ] **Delete `IDEA.md`** (unrelated Podman scratch). (`docs/bo-insstall.md` is already removed — done via the cleanup plan.)
