...
1. **`backoffice/ui/` violates the headless approach systematically.** 13 `useState` hooks, and `backoffice/ui/table-page.tsx` (545 lines) *is* the backoffice's real client core — query model, filters, sorting, debounce, editors, and error handling all live in React where Node tests can't reach them. Because coverage only includes `client/` and `server/` dirs, the coverage gate is structurally blind to all of it.
...
The plan below is ordered so that Milestone 1 fixes the headless violations and the worst UX bugs *in one move*: the blank screens exist precisely because error handling lives in components instead of state.
...
- [ ] **Extract the table query model out of `backoffice/ui/table-page.tsx` into `backoffice/client/`.** Move to state + events + actions: `query` (limit/offset/sort/dir/filters), `drafts`, `editor`, `error` (currently `useState` at `table-page.tsx:151-156`); `buildFilters` (`:31-68`), `toggleSort` (`:222-231`), `clearFilters` (`:266-271`), reset-on-table-change (`:162-173`, currently two `useRef` skip-flags), and URL mirroring (`:177-186`, belongs in the navigation loop over `domain-client/history.ts`). Write Node story tests for each flow.
...
- [ ] **Build a backoffice `attempt` equivalent** (mirror `app/client/attempt.ts`): errors become state, `AUTHENTICATION_REQUIRED` re-resolves auth, actions get pending keys. This removes the ad-hoc `load` callback in `backoffice/ui/app.tsx:22-36`, the private `run` re-implementation in `table-page.tsx:233-244`, and **fixes all four P0 blank-screen bugs**: failed `loadStatus` → blank page (`main.tsx:31`), silent row-load failures (`table-page.tsx:211-213`), stuck `Loading…` on detail pages (`user-detail-page.tsx:22-24`, `organization-detail-page.tsx:20-22`), silently empty sidebar (`app.tsx:39-41`).
...
- [ ] **Replace `window.confirm`** (`table-page.tsx:249`) with the `confirming`-state pattern and a `ConfirmButton` like `app/ui/confirm-button.tsx`.
- [ ] **Add loading/empty state to the table** as state, not JSX guesswork: today a table switch renders an empty `<tbody>` with no feedback (`table-page.tsx:216, 483-489`).
- [ ] **Move pure helpers out of `.tsx`**: `formatCell`/`rowText`/`rowKey` (`table-page.tsx:70-92`; `rowText` is already imported by `users-page.tsx`) into `backoffice/client/`, where coverage sees them. Same for `referencesTo` consumption (`backoffice/client/selectors.ts` is 0% covered because its only caller is UI).
...
- [ ] **Close the DOM-type loophole**: `app/tsconfig.json` and `backoffice/tsconfig.json` put `"DOM"` in `lib` while including `client/`, so client code compiles *with* DOM types — the no-DOM guarantee currently rests on ESLint alone. Split the client dirs into their own tsconfig without the DOM lib.
- [ ] **Raise coverage floors after Milestone 1** — once the backoffice logic is headless it enters `vitest.config.ts` coverage; ratchet the thresholds up. Also stop committing `coverage/` (it's 7 days stale in the tree) — add it to `.gitignore`.
...
- [ ] **Backoffice accessibility + theming**: zero ARIA today. Sortable headers are `<span onClick>` (`table-page.tsx:414-418`) — make them buttons with `aria-sort`; sidebar nav should be anchors with `aria-current`; label the per-column filter inputs; add focus management to the portalled popovers. Extract the duplicated design tokens (both `styles.css` files copy the same accent/shadows) into one shared token sheet, and give the backoffice the app's dark mode and a responsive breakpoint.
...
1. **Trim the domain?** Workspaces, work sessions, git materialization, secrets encryption, and the local git builder (~1,100 lines) are a *product*, not boilerplate. A cloner inherits all of it and must delete it. Consider a `template-minimal` branch or an init-script option that strips these aggregates.
...
- **Admin env-file write-back is fine as is.** `backoffice/server/env-file.ts` persists the one-time admin credential (email + password *hash*, never the password) into the local `.env` on first-run setup. The "immutable filesystem" concern only applied if the backoffice deployed to a server; as a local dev tool, a local file is the right store.
