...
- [ ] **Extract the table query model out of `backoffice/ui/table-page.tsx` into `backoffice/client/`.** Move to state + events + actions, with Node story tests for each flow. Split into landable slices:
  ...
  - [ ] Move `query` + `drafts` into a table-view state slice: events for open-table reset (`table-page.tsx:98-119`, currently `useState` + two `useRef` skip-flags), `toggleSort` (`:169-178`), `clearFilters` (`:213-218`), draft edits, filter apply (offset reset lives in the projection), and pagination; URL mirroring (`:123-133`) moves out of the UI effect into the pagination/query actions over `core.navigation.replace`. `table-page.tsx` keeps only a thin 300ms debounce effect that calls the apply action (a host timer capability is Milestone 2). Story tests per flow.
  - [ ] Move `editor` + `error` into the same slice (`table-page.tsx:102-103`): editor open/close events and mutation-error state replacing `setError` inside the private `run` wrapper (`:180-191`; `run`'s full removal belongs to the `attempt` task below). Story tests.
...
- [ ] **Build a backoffice `attempt` equivalent** (mirror `app/client/attempt.ts`): errors become state, `AUTHENTICATION_REQUIRED` re-resolves auth, actions get pending keys. This removes the ad-hoc `load` callback in `backoffice/ui/app.tsx:22-36`, the private `run` re-implementation in `table-page.tsx:180-191`, and **fixes all four P0 blank-screen bugs**: failed `loadStatus` → blank page (`main.tsx:31`), silent row-load failures (`table-page.tsx:157-159`), stuck `Loading…` on detail pages (`user-detail-page.tsx:22-24`, `organization-detail-page.tsx:20-22`), silently empty sidebar (`app.tsx:39-41`).
...
- [ ] **Replace `window.confirm`** (`table-page.tsx:196`) with the `confirming`-state pattern and a `ConfirmButton` like `app/ui/confirm-button.tsx`.
- [ ] **Add loading/empty state to the table** as state, not JSX guesswork: today a table switch renders an empty `<tbody>` with no feedback (`table-page.tsx:163, 430-436`).
- [ ] **Move pure helpers out of `.tsx`**: `formatCell`/`rowText`/`rowKey` (`table-page.tsx:25-47`; `rowText` is already imported by `users-page.tsx`) into `backoffice/client/`, where coverage sees them. Same for `referencesTo` consumption (`backoffice/client/selectors.ts` is 0% covered because its only caller is UI).
...
- [ ] **Backoffice accessibility + theming**: zero ARIA today. Sortable headers are `<span onClick>` (`table-page.tsx:360-368`) — make them buttons with `aria-sort`; sidebar nav should be anchors with `aria-current`; label the per-column filter inputs; add focus management to the portalled popovers. Extract the duplicated design tokens (both `styles.css` files copy the same accent/shadows) into one shared token sheet, and give the backoffice the app's dark mode and a responsive breakpoint.
