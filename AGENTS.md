...
4. **Throw `AppError` anywhere a request can reach** ([domain-server/errors.ts](domain-server/errors.ts)), never bare `Error` — its code and status are what the error handler answers with. Outside the request path (startup, shutdown, key construction) a bare `Error` is fine; the status would be meaningless. Never put SQL, credentials, stack traces, or upstream exception text in a message.
